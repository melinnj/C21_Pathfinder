...
uri = uri_helper.uri_from_env(default="radio://0/12/2M/EE5C21CF11")
...
# Map the grid and navigate white tiles
def navigate_white_tiles_from_start(grid_size, scf, pc, start_position):
    rows, cols = grid_size
    grid_map = np.full((rows, cols), -1)  # -1 indicates unmapped cells
    log_conf = LogConfig(name="PositionAndLight", period_in_ms=100)
    log_conf.add_variable("stateEstimate.x", "float")
    ...
    while stack:
        ...
        if current_cell == start_cell:
            # Assume the starting cell is white
            cell_value = 0
        else:
            cell_value = 1 if light_level > LIGHT_THRESHOLD else 0
        grid_map[grid_y, grid_x] = cell_value
...
if __name__ == "__main__":
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache="./cache")) as scf:
        ...
        with PositionHlCommander(
            ...
            print("Navigating white tiles from start point...")
            # Navigate and map the grid
            grid_map = navigate_white_tiles_from_start(grid_size, scf, pc, start_position)
